...
###   This script attempts to download the signature file SHA256SUMS.asc from kazusilver.org
...
WORKINGDIR="/tmp/kazusilver"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://kazusilver.org/bin/"
VERSIONPREFIX="kazusilver-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'kazusilver-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the KazuSilver Core binary release signing key installed?"
   fi
