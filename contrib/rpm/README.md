...
The RPM spec file provided here is for KazuSilver-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://kazusilver.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[KazuSilver git repository](https://github.com/kazusilver/kazusilver) but are not part of
...
Currently only `Source10` is used, for the example `kazusilver.conf` file.
...
in the kazusilver git repository. At present only an SVG version of the KazuSilver
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the kazusilver
...
needed when 0.12.1 is released, a proper fix is already in the KazuSilver git
master branch.
...
idea to build KazuSilver Core against a static version of that library compiled
...
The `kazusilver.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `kazusilver-server` package. At this point, most RPM based
...
The `kazusilver-server` package should have SELinux support. How to properly do
...
KazuSilver-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/kazusilver/). He has been
packaging KazuSilver for Fedora at least since 2012.
...
4. I renamed the `kazusilver` package that contains the Qt GUI to `kazusilver-core` as
that appears to be how the general population refers to it, in contrast to
`kazusilver-xt` or `kazusilver-classic`. I wanted to make sure the general population
