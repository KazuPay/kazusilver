    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return KazuSilverUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, KazuSilverUnits::separatorNever);
            else
                return KazuSilverUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != NULL) ? tr("Requested") + " ("+KazuSilverUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
