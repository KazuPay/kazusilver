...
const std::string KazuSilverGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString KazuSilverGUI::DEFAULT_WALLET = "~Default";

KazuSilverGUI::KazuSilverGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void KazuSilverGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/kazusilver"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a KazuSilver address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kazusilver: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your KazuSilver addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified KazuSilver addresses"));
    ...
    openAction->setStatusTip(tr("Open a kazusilver: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible KazuSilver command-line options").arg(tr(PACKAGE_NAME)));
...
void KazuSilverGUI::bootstrapBlockchain()
{
    bool ok = false;
    QString defaultUrl = "https://nav.nyc3.digitaloceanspaces.com/bootstrap/mainnet/bootstrap-kazusilver_" +
            QString::fromStdString(Params().NetworkIDString()) + "net.tar";
    QString url = QInputDialog::getText(this, tr("Bootstrap blockchain"),
                                            tr("You can use an external trusted source to download the blockchain from.<BR>The following URL points to a bootstrap copy provided by the KazuSilver Core Team.<BR>Where would you like to download it from?"), QLineEdit::Normal,
                                            defaultUrl, &ok);
    if (ok && !url.isEmpty())
    {
...
void KazuSilverGUI::createMenuBar()
{
#ifdef Q_OS_MAC
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(KazuSilverUnits::Unit u, KazuSilverUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(KazuSilverUnits::name(u)), this);
...
void KazuSilverGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void KazuSilverGUI::createToolBars()
{
    if(walletFrame)
    {
...
void KazuSilverGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool KazuSilverGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void KazuSilverGUI::startVotingCounter()
{
    if (GetStaking())
    {
...
bool KazuSilverGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void KazuSilverGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void KazuSilverGUI::repairWallet()
{
...
void KazuSilverGUI::setWalletActionsEnabled(bool enabled)
{
...
void KazuSilverGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void KazuSilverGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void KazuSilverGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void KazuSilverGUI::optionsClicked()
{
...
void KazuSilverGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void KazuSilverGUI::showDebugWindow()
{
...
void KazuSilverGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void KazuSilverGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void KazuSilverGUI::openClicked()
{
...
void KazuSilverGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void KazuSilverGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KazuSilverGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KazuSilverGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KazuSilverGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KazuSilverGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KazuSilverGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void KazuSilverGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to KazuSilver network", "", count));
    if(walletFrame){
...
void KazuSilverGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to KazuSilver network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to KazuSilver network..."));
...
void KazuSilverGUI::showVotingDialog()
{
...
void KazuSilverGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("KazuSilver"); // default title
...
void KazuSilverGUI::changeEvent(QEvent *e)
{
...
void KazuSilverGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void KazuSilverGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KazuSilverGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(KazuSilverUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void KazuSilverGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KazuSilverGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KazuSilverGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KazuSilverGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KazuSilverGUI::setEncryptionStatus(int status)
{
...
void KazuSilverGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KazuSilverGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KazuSilverGUI::detectShutdown()
{
...
void KazuSilverGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void KazuSilverGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(KazuSilverGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void KazuSilverGUI::subscribeToCoreSignals()
{
...
void KazuSilverGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<KazuSilverUnits::Unit> units = KazuSilverUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const KazuSilverUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(KazuSilverUnits::name(unit)));
    }
...
{
    setText(KazuSilverUnits::name(newUnits));
}
...
void KazuSilverGUI::toggleStaking()
{
...
void KazuSilverGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void KazuSilverGUI::updatePrice()
{
...
void KazuSilverGUI::replyFinished(QNetworkReply *reply)
{
...
void KazuSilverGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CKazuSilverAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void KazuSilverGUI::getVotingInfo()
{
    QNetworkAccessManager *manager = new QNetworkAccessManager();
    QNetworkRequest request;
    QNetworkReply *reply = NULL;
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.kazusilver.org/voting.") + QString::fromStdString(Params().NetworkIDString()) + QString("net.json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void KazuSilverGUI::updateStakingStatus()
{
    updateWeight();
