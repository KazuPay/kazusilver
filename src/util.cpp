...
const char * const KSLVCOIN_CONF_FILENAME = "kazusilver.conf";
const char * const KSLVCOIN_PID_FILENAME = "kazusilver.pid";
...
#else
    const char* pszModule = "kazusilver";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KazuSilver
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KazuSilver
    // Mac: ~/Library/Application Support/KazuSilver
    // Unix: ~/.kazusilver
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KazuSilver";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KazuSilver";
#else
    // Unix
    return pathRet / ".kazusilver";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", KSLVCOIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No kazusilver.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override kazusilver.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", KSLVCOIN_PID_FILENAME));
